...
### Note importante pour ce repo

La configuration **Web** du projet `huit-americain` est maintenant pré-remplie dans `lib/firebase_config.dart`.
Donc, pour ce projet précis, tu peux faire un simple:

```bash
flutter run -d chrome
```

Les `--dart-define=FIREBASE_WEB_*` restent possibles si tu veux surcharger ces valeurs.

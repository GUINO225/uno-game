...
class GameService {
  ...
  Future<void> exitBetParty({
    ...
    await db.runTransaction((Transaction tx) async {
      ...
      final DuelSession session = DuelSession.fromDoc(snap);
      if (!session.players.contains(playerId)) return;
      tx.update(ref, <String, dynamic>{
...
class DuelController extends ChangeNotifier {
  ...
  OpponentConnectionStatus get opponentConnectionStatus {
    ...
    if (opponentId.isEmpty) return OpponentConnectionStatus.online;

    final DuelPlayerPresence presence =
        current.presence[opponentId] ?? const DuelPlayerPresence();

    // Quitter volontairement l'écran de jeu
    ...
    if (elapsed >= _connectionWarningTimeout ||
      ...
      return OpponentConnectionStatus.unstable;
    }
    return OpponentConnectionStatus.online;
  }
  ...
  Future<void> applyCardBonusCredits({
    ...
    await service.applyCardBonusCredits(
      current: current,
      winnerId: winnerId,
...
class _DuelLobbyPageState extends State<DuelLobbyPage> {
  ...
  Future<void> _copyGameCode(String gameCode) async {
    unawaited(_sfx.playClick());
                          ...
                          child: Column(
                            ...
                            children: <Widget>[
                              ...
                              if (session != null) ...<Widget>[
                                const SizedBox(height: 16),
                                _DuelLobbyCreatedCodeCard(
                                  gameCode: session.gameId,
                                  accent: accent,
                                  secondaryAccent: secondaryAccent,
                                  onCopy: () => _copyGameCode(session.gameId),
                                ),
                              ],
                            ],
                          ),
  ...
  Widget build(BuildContext context) {
    return AnimatedContainer(
      ...
      decoration: BoxDecoration(
        ...
        gradient: LinearGradient(
          ...
          colors: <Color>[
            ...
            secondaryAccent.withOpacity(0.14),
          ],
        ),
        boxShadow: <BoxShadow>[
          BoxShadow(
      ...
      child: Column(
        ...
        children: <Widget>[
          Text(
            'Copiez le code et envoyez-le à votre adversaire.',
            style: TextStyle(
...
class _DuelPageState extends State<DuelPage> with WidgetsBindingObserver {
  ...
  bool _maybeNotifyPresenceChange(DuelSession session) {
    ...
    final String opponentName = session.playerNames.entries
        .firstWhere(
          (MapEntry<String, String> e) => e.key != _controller.localPlayerId,
          orElse: () => const MapEntry<String, String>('', 'Adversaire'),
        )
        .value;
    ...
    switch (current) {
      case OpponentConnectionStatus.away:
        FunnyGameToast.show(
          ...
          message: 'a quitté l\'écran de jeu.',
          type: FunnyMessageType.difficulty,
          duration: const Duration(seconds: 4),
        );
      case OpponentConnectionStatus.unstable:
        FunnyGameToast.show(
          context,
          playerName: opponentName,
          message: 'a une connexion instable...',
          type: FunnyMessageType.difficulty,
          duration: const Duration(seconds: 4),
        );
      case OpponentConnectionStatus.disconnected:
        FunnyGameToast.show(
          context,
          playerName: opponentName,
      ...
      case OpponentConnectionStatus.online:
        FunnyGameToast.show(
          context,
          playerName: opponentName,
          message: 'est de retour !',
...
class _OpponentPresenceBadge extends StatefulWidget {
  ...
  final OpponentConnectionStatus status;

  @override
  State<_OpponentPresenceBadge> createState() => _OpponentPresenceBadgeState();
}

class _OpponentPresenceBadgeState extends State<_OpponentPresenceBadge>
  ...
  @override
  void didUpdateWidget(covariant _OpponentPresenceBadge oldWidget) {
    super.didUpdateWidget(oldWidget);
